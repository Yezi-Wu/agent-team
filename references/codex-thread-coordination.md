...
2. Create only Product, Design, Engineering, and QA tasks in the same local project directory when those roles are needed. Before dispatch, create a compact versioned task packet that lists the exact allowed read paths and expected outputs. Require an `accepted` receipt before dependent work starts. Their prompts must use the concise handoff structure and must not request a full repository scan.
3. Start Product immediately. Start Design, Engineering, and QA in waiting state: they must not invent missing inputs.
4. When Product finishes, record a coordinator trace entry, update the relevant project map, and request user approval if requirements affect scope, budget, dates, or material commitments. Then create Design's task packet with product-file versions and wait for Design's `accepted` receipt before work begins.
